...
> three retrieval collections are empty. Improving the encoder would move a number that does not
...
- **`AutoDocEngine` can write one client's drawing text into another client's rule file, and a
  database hiccup can write a rule from a single dismissal.** *(Re-homed here 2026-08-10 from the
  retired R3 — [[ADR-009 Retiring the Standards Knowledge Track]]. Unfixed.)* Two defects in six lines of
