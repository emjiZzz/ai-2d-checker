...
def execute_crop_verification(api_key: str, batch: list[CropVerificationInput]) -> dict:
    ...
    system_instruction = (
        ...
        "IMPORTANT: Adding, removing, or updating engineering standard symbols (such as diameter Ø/⌀/ø, "
        "radius R, chamfer C, degree °, surface finish, or welding symbols) IS a real visual difference "
        "(differs=True and status=CHANGED). Never classify symbol additions or updates as identical/MATCHED. "
        "Conversely, Shim Tables (`シム表` / Shim Schedule listing thickness `t`, material, quantity, `設計組厚サ`, `総厚サ`) "
        "are auxiliary reference tables and are explicitly out of scope — do not verify or emit findings for them. "
