...
#: other Mongo-sourced collections, and kept in its own collection rather than merged into
#: `CORRECTIONS` because the two answer different questions. `CORRECTIONS` and `FINDINGS` are
#: anchored to engine output -- `AuditFeedbackDocument` requires `original_status`, "Status
#: generated by AI engine", and every correction verb is an override of a finding the engine
#: produced -- so neither can contain something the engine never reported. A marking is made
#: against the drawing with no engine output on screen, so it can. Merging the pools would let a
#: dismissal answer a question about what is on the sheet, which is how the engine's own recall
#: boundary would come back through the retrieval path.
#: Pinned by `tests/test_retrieval_ground_truth_pool.py`.
