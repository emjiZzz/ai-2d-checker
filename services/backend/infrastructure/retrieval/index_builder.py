...
VAULT = "vault"
ENTITIES = "entities"
#: Independent human ground truth from the prototype-mode Manual Check, client-local like the
#: other Mongo-sourced collections. Its own pool, not merged into `CORRECTIONS`: that one is
#: engine-anchored and so cannot hold a finding the engine never reported, while this one can.
#: See [[Gotcha - The Ground Truth Store the RAG Could Not Read]]. Pinned by
#: `tests/test_retrieval_ground_truth_pool.py`.
